...
def parse_args():
    ...
    parser.add_argument('--write-summary-stats',
                        type=bool,
                        help='Write summary statistics?',
                        default=True)
    parser.add_argument('--summary-stat-output',
                        type=str,
                        help='File to write summary statistics to',
                        default='output')
    parser.add_argument('--write-seqs',
                        type=bool,
                        help='Write simulated sequences?',
                        default=False)
    parser.add_argument('--sequence-output',
                        type=str,
                        help='File to write the simulated sequences to',
                        default='')
    parser.add_argument('--ber-lambda',
                        type=float,
                        default=.5)
    parser.add_argument('--bubble-size',
                        type=int,
                        default=20)
    parser.add_argument('--exo-left',
                        type=float,
                        default=.2)
    parser.add_argument('--exo-right',
                        type=float,
                        default=.2)
    parser.add_argument('--aid-context-model',
                        type=str,
    ...
    parser.add_argument('--context-model-length',
                        type=int,
                        help='Length of the context for the context model',
                        default=3)
    parser.add_argument('--context-model-pos-mutating',
                        type=int,
                        help='Position mutating in the context model',
                        default=2)
    parser.add_argument('--germline-sequence',
                        type=str,
                        help='Starting sequence',
                        default='/Users/juliefukuyama/GitHub/HyperMutationModels/data/gpt.fasta')
    parser.add_argument('--n-mutation-rounds',
                        type=int,
                        default=3)
    parser.add_argument('--n-seqs',
                        type=int,
                        default=500)
    parser.add_argument('--ber-params',
                        type=str,
                        default='[.25,.25,.25,.25]')
    parser.add_argument('--pol-eta-a',
                        type=str)
    parser.add_argument('--pol-eta-g',
                        type=str)
    parser.add_argument('--pol-eta-c',
                        type=str)
    parser.add_argument('--pol-eta-t',
                        type=str)
    parser.add_argument('--p-fw',
                        type=float,
                        default=.5)
...
def main(args=sys.argv[1:]):
    ...
    pol_eta_params = {'A': ast.literal_eval(args.pol_eta_a),
    ...
    sequence = list(SeqIO.parse(args.germline_sequence, "fasta",
                                alphabet=IUPAC.unambiguous_dna))[0]
    mutated_seq_list = []
    mmr_length_list = []
    aid_model_string = pkgutil.get_data("SHMModels", args.aid_context_model)
    aid_model = ContextModel(args.context_model_length,
                             args.context_model_pos_mutating,
                             aid_model_string)
    for i in range(args.n_seqs):
        mr = MutationRound(sequence.seq,
                           ...
                           aid_time=10,
                           exo_params={'left': args.exo_left,
                                       'right': args.exo_right},
