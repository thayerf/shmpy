...
def sample_prior():
    ...
    rate = np.random.uniform(low = 0.25, high = 4.0)
    var = np.random.uniform(low = 0.25, high = 2.0)
...
def forward_sample_sequence_sgcp(sequence, params):
    n = np.size(sequence)
    re = np.random.normal(scale = params['re_var'])
    vals = np.random.poisson(lam = np.exp(np.log(params['base_rate']) + re), size = n)
...
def importance_sample(obs_sequences,n_imp_samp, n, eps):
    ...
    for i in range(n_imp_samp):
        ...
        re_list.append(model_params['re_var'])
        if i % 50 == 0:
            print(i)
    return rate_list, ls_list, sg_list, re_list, w_list, base_colocal

true_model_params = sample_prior()
obs_sample,A, A_t, g = gen_batch(germline,true_model_params, 300)

rate_list, ls_list, sg_list, re_list, w_list, base_colocal = importance_sample(obs_sample, 1000, 300, 0.75)
...
pred_mean_re = np.dot(w_list, re_list)/np.sum(w_list)
